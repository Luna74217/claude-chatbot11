...
class ClaudeClient:
    ...
    async def get_response(self, user_message: str, model: str = "claude-opus-4-1-20250805", context_messages: Optional[List[Dict[str, str]]] = None) -> str:
        try:
            # 컨텍스트 메시지가 있으면 포함
            messages = []
            if context_messages:
            ...
            response = await loop.run_in_executor(
                ...
                lambda: self.client.messages.create(
                    model=model,
                    max_tokens=4096,  # Opus 4.1은 더 긴 출력 지원 (최대 32K)
                    messages=messages
                )
            )
    ...
    async def get_streaming_response(self, user_message: str, model: str = "claude-opus-4-1-20250805") -> AsyncGenerator[str, None]:
        """스트리밍 응답 생성 (시뮬레이션)"""
        try:
            # 실제 Claude API 호출
    ...
    async def get_transformed_stream(self, user_message: str, transformer_configs: Optional[list[Dict[str, Any]]] = None, model: str = "claude-opus-4-1-20250805") -> AsyncGenerator[str, None]:
        """변환기가 적용된 스트리밍 응답"""
        # 기본 변환기 설정
